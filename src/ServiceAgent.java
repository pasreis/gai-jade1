...
public class ServiceAgent extends Agent {
	protected void setup () {
		...
		addBehaviour(new AdditionalAgentCyclicBehaviour(this));
		//doDelete();
	}
...
class AdditionalAgentCyclicBehaviour extends CyclicBehaviour
{
	ServiceAgent agent;
	public AdditionalAgentCyclicBehaviour(ServiceAgent agent)
	{
		this.agent = agent;
	}
	public void action()
	{
		MessageTemplate template = MessageTemplate.MatchOntology("eng-por");
		ACLMessage message = agent.receive(template);
		if (message == null)
		{
			block();
		...
		else
		{
			...
			try
			{
				response = agent.makeRequest("fd-eng-por",content);
			}
			catch (NumberFormatException ex)
			{
				response = ex.getMessage();
